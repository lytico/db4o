...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1">
 ...
 <xsl:include href="htmlBody.xsl"/>
  ...
  <xsl:template name="body">
    ...
    <xsl:variable name="obsolete" select="/document/reference/attributes/attribute[type/@api='T:System.ObsoleteAttribute']" />
	<xsl:if test="$obsolete">
	  <xsl:choose>
		<xsl:when test="normalize-space($obsolete/argument/value)">
		  <b>(Obsolete: <xsl:value-of select="$obsolete/argument/value" />)</b>
		</xsl:when>
		<xsl:otherwise>
		  <b>(This API is obsolete!)</b>
		</xsl:otherwise>
	  </xsl:choose>
	</xsl:if>    
		<!-- auto-inserted info -->
