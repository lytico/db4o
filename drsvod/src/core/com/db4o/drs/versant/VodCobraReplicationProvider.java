...
public class VodCobraReplicationProvider implements TestableReplicationProviderInside, LoidProvider {
	...
	private TimestampGenerator _timestampGenerator;

	private ObjectInfoMaintainer _objectInfoMaintainer;

	public VodCobraReplicationProvider(VodDatabase vod) {
		_vod = vod;
		_cobra = VodCobra.createInstance(vod);
		CobraReplicationSupport.initialize(_cobra);
		_timestampGenerator = new TimestampGenerator(_cobra);
		_objectInfoMaintainer = new ObjectInfoMaintainer(_cobra);
	...
	public void startReplicationTransaction(ReadonlyReplicationProviderSignature peer) {
		...
		if(_replicationCommitRecord == null){
			...
			_timestampGenerator.setMinimumNext(_replicationCommitRecord.timestamp() + 1);
			if(_replicationCommitRecord.concurrentTimestamps() == null){
				_replicationCommitRecord.concurrentTimestamps(new long[]{});
			}
		}
		_commitTimestamp = _timestampGenerator.generate();
	}
	...
	public void storeReplica(Object obj){
		...
		if (!(obj instanceof PersistenceCapable)) {
			throw new IllegalArgumentException(VodCobraReplicationProvider.class.getSimpleName()+" can only handle " + PersistenceCapable.class.getSimpleName() + " objects");
		}
	...
	private static final Class[] knownClasses(){
		...
		// temporary hack to get a simple test going
		// we need to get the known classes from JPA
		// or the user would have to specify known classes
...
//	}
	...
	public void syncCommitTimestamp(long syncedTimeStamp) {
		...
		// We think we don't need this call. We only generate the _commitTimestamp once 
		// in the beginning and then we may be synced in this call, that's it.
		// _timestampGenerator.setMinimumNext(syncedTimeStamp);
